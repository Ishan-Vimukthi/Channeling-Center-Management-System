...
if(strlen($_SESSION['id']) == 0) {
    ...
    if(isset($_POST['submit'])) {
        ...
        $stmt = $con->prepare("CALL AddDoctor(?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssi", $docspecialization, $docname, $docaddress, $docfees, $doccontactno, $docemail, $password);

        if ($stmt->execute()) {
            echo "<script>alert('Doctor info added Successfully');</script>";
            echo "<script>window.location.href ='manage-doctors.php'</script>";
        } else {
            echo "<script>alert('Error adding doctor info');</script>";
        }
