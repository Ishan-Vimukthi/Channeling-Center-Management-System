...
if (strlen($_SESSION['id']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        // Retrieve form data
        $eid = $_GET['editid']; // Patient ID for the record to update
        ...
        // Call the stored procedure using MySQLi
        $stmt = $con->prepare("CALL UpdatePatient(?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssiss", $eid, $patname, $patcontact, $patemail, $gender, $pataddress, $patage, $medhis);
